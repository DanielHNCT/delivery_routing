...
use crate::cache::{AuthCache, TourneeCache, RedisClient, CacheConfig, CacheOperations};
...
impl Default for MigrationConfig {
    fn default() -> Self {
        Self {
            current_strategy: MigrationStrategy::MobileOnly, // Cambiado de WebOnly a MobileOnly
...
impl MigrationService {
    ...
    pub async fn change_strategy(&mut self, new_strategy: MigrationStrategy) -> Result<()> {
        ...
        info!("✅ Estrategia cambiada exitosamente a {:?}", self.config.current_strategy);
        Ok(())
    }
    ...
    pub async fn get_current_strategy(&self) -> Result<MigrationStrategy> {
        let strategy = self.config.current_strategy;
        debug!("🔍 Estrategia actual solicitada: {:?}", strategy);
        Ok(strategy)
    }
